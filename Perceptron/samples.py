...
def generateSamples(N, numTraining, test=False):
	...
	trainImages = np.zeros((numTraining, N2))
	...
	while(j < numTraining):
		...
		xLabel_prev_vec = np.reshape(xLabel_prev, (1,N2))
		xDistract_vec = X_vec - xLabel_vec
		X_vec[X_vec == 0] = -1
		xLabel_vec[xLabel_vec == 0] = -1
		xLabel_prev_vec[xLabel_prev_vec == 0] = -1
		xDistract_vec[xDistract_vec == 0] = -1

		trainImages[j, :] = X_vec
		trainFeatures[j, :] = xLabel_prev_vec
	...
	trainDict = {"Features": trainFeatures, "Labels": trainLabels, "Distractors": trainDistract, "Images": trainImages}
	return trainDict
